...
export abstract class EntityContainer<
  T extends EntityContext<Entity<EntityLinks>, SearchCriteria>,
  U extends SearchEvent<SearchCriteria>,
> extends BaseComponent {
  ...
  private readonly page$: Observable<Page<ListWrapper>> =
    this.searchEventSubject.pipe(
      switchMap((e) => this.run(this.entityService.read(e))),
    );
  ...
  protected abstract retrieveEntities(
    page: Page<ListWrapper>,
  ): Entity<EntityLinks>[];
  protected abstract getEntityListPath(): string;

  protected subscribeToEntityListViewEvents(
    component: EntityListViewComponent<Entity<EntityLinks>, T, U>,
  ) {
    this.page$
      .pipe(
        takeUntil(component.destroy$),
        tap((page) => this.onPage(page)),
      )
      .subscribe();
    component.manage
      .pipe(
        takeUntil(component.destroy$),
        switchMap((event) => this.writeEntity(event)),
        tap(() => this.search()),
      )
      .subscribe();
    component.search
      .pipe(
        takeUntil(component.destroy$),
        debounceTime(1000),
        tap((event) => this.searchEventSubject.next(event)),
      )
      .subscribe();
    component.pageChange
      .pipe(
        takeUntil(component.destroy$),
        debounceTime(1000),
        tap((pageNumber) => this.onPageChange(pageNumber)),
      )
      .subscribe();
    component.sort
      .pipe(
        takeUntil(component.destroy$),
        tap((event) => this.onSort(event)),
      )
      .subscribe();
    component.select
      .pipe(
        takeUntil(component.destroy$),
        tap((entity) => this.onSelect(entity)),
      )
      .subscribe();
  }

  protected subscribeToEntityViewEvents(
    component: EntityViewComponent<Entity<EntityLinks>, T>,
  ) {
    component.manage
      .pipe(
        takeUntil(component.destroy$),
  ...
  private readEntity(): void {
    ...
    const obs$ = this.entityService.readByUrl(
      queryParams[this.getEntityUriParamName()],
    );
  ...
  private writeEntity(
    event: EntityEvent<Entity<EntityLinks>>,
  ): Observable<void> {
  ...
  private subscribeToEvents(component: ViewComponent) {
    if (
      component instanceof EntityListViewComponent ||
      component instanceof EntityViewComponent
    ) {
      this.busySubject
        .pipe(
          takeUntil(component.destroy$),
          tap((busy) => (component.busy = busy)),
        )
        .subscribe();
    }
    if (component instanceof EntityListViewComponent) {
  ...
  private onPage(page: Page<ListWrapper>): void {
    ...
    this.updateContext({
      entities,
      pagination: PageUtil.getPagination(page),
    } as Partial<T>);
  }
        ...
        tap((searchEvent) => {
          ...
          if (!event) {
            ...
            event = {
              ...
              pageRequest: context.pagination?.request,
            } as U;
          }
  ...
  protected onReadEntity(
    entity: Entity<EntityLinks>,
  ): Observable<Entity<EntityLinks>> {
    return of(entity);
  }
