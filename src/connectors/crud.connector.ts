...
export abstract class CrudConnector<T, U extends SearchCriteria, V extends ListWrapper> extends ApiConnector {
	...
	read(searchCriteria: U, pageRequest: PageRequest, url?: string): Observable<Page<V>> {
		const endpoint = url || this.getReadUrl();
		return this.httpClient.post<Page<V>>(endpoint, searchCriteria, this.getOptions(pageRequest));
	}
